...
namespace TTT.Physics
{
    public class CylinderCollider : MonoBehaviour
    {
        ...
        {
            return UnityEngine.Physics.CheckBox(position, new Vector3(radius, height, radius)) &&
                UnityEngine.Physics.CheckCapsule(position - new Vector3(0, height, 0), position + new Vector3(0, height, 0), radius);
        }

        public static bool CheckCylinder(Vector3 position, float height, float radius, Quaternion rotation, LayerMask mask)
        {
            return UnityEngine.Physics.CheckBox(position, new Vector3(radius, height, radius), rotation, mask) &&
                UnityEngine.Physics.CheckCapsule(position - new Vector3(0, height, 0), position + new Vector3(0, height, 0), radius, mask);
        }
        ...
        void OnValidate()
        {
            // 부모와 모든 자식의 Collider 컴포넌트를 가져옵니다.
            Collider[] colliders = GetComponentsInChildren<Collider>();
            // 각 Collider에 물리 재질을 적용합니다.
